...
#
# Same as main text fig 6 w/testing everyone
# _____________________________________________________________________
...
# _____________________________________________________________________
#* I: Get data ####
#* If you want to generate data yourself, start here! Otherwise, skip to line 94 to use pre-generated dataframes.
# _____________________________________________________________________
ptm <- proc.time()
...
#saveRDS(df,file="df_SuppFig5.RData")
...
plot_timeseries <- ggplot(df_timeseries, aes(x = time)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 3500/1000)) +
  alllabels_theme +
  coord_fixed(200/3.5) +
...
suppfig5 <- arrangeGrob(panels, percent_legend, plot_timeseries, layout_matrix = lay,
                        widths = c(2.8, 0.2, 1))
plot(suppfig5)

ggsave("SuppFig5.pdf", suppfig4, device = cairo_pdf, width = 8, height = 2.5)
ggsave("SuppFig5.svg", suppfig4, device = svg, width = 8, height = 2.5)
